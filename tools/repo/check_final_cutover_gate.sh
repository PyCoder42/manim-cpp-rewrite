...
if ! rg -n "LegacyArtifactsPurgeCheck" "${tests_cmake_path}" >/dev/null 2>&1; then
  echo "Final cutover requires legacy artifact purge enforcement coverage." >&2
  exit 1
fi

if ! rg -n "NoRstArtifactsCheck" "${tests_cmake_path}" >/dev/null 2>&1; then
  echo "Final cutover requires mdBook-only docs artifact enforcement coverage." >&2
  exit 1
fi
